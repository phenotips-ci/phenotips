define([
    ...
    PedigreeImport.initFromInternalUndoRedoJSON = function(inputText)
    {
        try {
            var input = JSON.parse(inputText);
        } catch(err) {
            throw "Unable to import pedigree: input is not a valid pedigree internal JSON: " + err.message;
        }

        if (!input.hasOwnProperty("layout") || !input.hasOwnProperty("proband")) {
            throw "Unable to import pedigree: input is mising required data (layout and proband)";
        }

        var newG = new BaseGraph();
        newG.fromJSON(input["baseGraph"]);

        return {"baseGraph": newG, "probandNodeID": input["proband"], "layout": input["layout"]};
    },
